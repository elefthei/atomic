...
export function buildProcessUnreadPrompt(args: {
  ...
  return [
    ...
    "You are the decision-maker: use snapshots, screenshots, and visible browser state to read the unread message, inspect the sender profile when applicable, and generate a natural follow-up from the template.",
    ...
    "",
    "<message_quality>",
    "Your job: write a LinkedIn DM reply that sounds like you read both the sender's unread message and their profile. Use the template as the structure and voice, not as a carbon copy.",
    "",
    "Rules:",
    "- First understand what the sender is asking/saying in the unread message and directly address it.",
    "- Fill each [slot] from the unread message and visible profile content. [name] and [Name] both mean first name, capitalized.",
    "- Use the same profile-inspection depth as the linkedin-connect workflow: visible Experience, About, Featured, and Activity content when available.",
    ...
    "  d. Read the unread message(s) and enough recent thread context to understand what the sender is saying. If this is a group conversation or the sender is ambiguous, skip with reason `group_or_ambiguous_sender`.",
    ...
    "",
    "Step 3 — inspect the sender profile when applicable.",
    "  a. Open the sender's visible profile link/name/avatar from the conversation. If no profile path is available, continue from message context only and record `profile_unavailable` in personalizationNotes.",
    `  b. Save screenshots to ${args.artifactsDir}/message-${n}-profile-top.png and, after scrolling, ${args.artifactsDir}/message-${n}-profile-details.png when a profile opens.`,
    "  c. Read visible headline, company/title, Experience, About, Featured, and Activity when available. Do not scrape hidden DOM or use brittle selectors.",
    ...
    "  a. Read the template file, generate a natural response per <message_quality>, and verify no literal [bracketed] placeholders remain.",
