...
- Broker stderr is no longer discarded. It is captured to `broker.log` in the agent's intercom directory through an already-open file descriptor (not a pipe, which cannot outlive the parent that spawned the detached broker), and both the "exited before startup" error and the readiness-timeout error now quote the log path and a bounded tail of that output. The Windows hidden launcher appends the broker's own stderr to the same file ([#2208](https://github.com/bastani-inc/atomic/issues/2208)).
- The startup log is capped at 8 KiB rather than growing without limit. The parent truncates it on every spawn, but the parent exits while the detached broker keeps running, so the broker applies the cap itself. External truncation would not work — a POSIX append descriptor writes at the new end of file, and the inherited Windows append handle has no truncate right ([#2208](https://github.com/bastani-inc/atomic/issues/2208)).
- The cap is installed by the broker entrypoint's first import, before any other module of the broker evaluates. ESM runs a module's static dependencies ahead of the importer's own body, so installing from the entrypoint body left every dependency free to write first: an oversized write while a dependency initialized produced a 32,768-byte log, and a throw during initialization produced 34,372 bytes ([#2208](https://github.com/bastani-inc/atomic/issues/2208)).
- The cap covers all three routes that reach the log, not only the stream: `process.stderr.write`, `console.error` / `console.warn`, and the default fatal printing for an uncaught exception or an unhandled rejection. Measured against an 8 KiB cap with only the stream patched, a Bun `console.error` left 32,769 bytes, a Bun uncaught error 33,228 bytes, a Bun unhandled rejection 33,326 bytes, and a Node unhandled rejection 33,620 bytes; all four now stay at or below the cap ([#2208](https://github.com/bastani-inc/atomic/issues/2208)).
- Broker readiness now polls at 10 ms with bounded backoff up to the previous 100 ms interval instead of sleeping a flat 100 ms after every attempt, which had become the dominant cost of a startup that finishes in tens of milliseconds. The overall startup timeout is unchanged ([#2208](https://github.com/bastani-inc/atomic/issues/2208)).
